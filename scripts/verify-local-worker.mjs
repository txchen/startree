...
const verifyLocalWorker = async (scenario) => {
  const port = scenarioPorts.get(scenario);
  ...
  try {
    ...
    if (scenario === 'management') {
      ...
      if (!clientResponse.headers.get('content-security-policy')?.includes("default-src 'self'")) {
        throw new Error(
          'The local Worker did not apply baseline security headers to client assets.',
        );
      }
    }
    ...
    try {
      ...
      if (scenario === 'management') {
        ...
        await page
          .locator('.folder-grid')
          .getByText('UI Folder Renamed', { exact: true })
          .waitFor();
        ...
        if (
          !(await page.getByLabel('URL').evaluate((element) => element === document.activeElement))
        ) {
        ...
        if (page.url() !== editModeUrl) {
          throw new Error(
            'Bookmark activation navigated while the Bookmarks Page was in Edit Mode.',
          );
        }
        ...
        await page.locator('.write-status.pending').waitFor({ state: 'detached' });
        const reorderedSnapshot = await fetch(
        ...
        let continueSlowUpdate;
        let resolveSlowUpdateIntercepted;
        const slowUpdateIntercepted = new Promise((resolve) => {
          resolveSlowUpdateIntercepted = resolve;
        });
        await page.route('**/api/bookmarks/commands', async (route) => {
          await new Promise((continueRequest) => {
            continueSlowUpdate = continueRequest;
            resolveSlowUpdateIntercepted();
          });
        ...
        await page.route('**/api/bookmarks/commands', async (route) => {
          ...
          const concurrentResponse = await fetch(
            `http://127.0.0.1:${port}/api/bookmarks/commands`,
            {
              method: 'POST',
          ...
          if (!concurrentResponse.ok) {
            throw new Error(
              `Concurrent conflict fixture failed with ${concurrentResponse.status}.`,
            );
          }
          await route.continue();
        });
        ...
        await page.waitForFunction(() =>
          [...document.querySelectorAll('.folder-tile')][0]?.textContent?.includes(
            'UI Folder Renamed',
          ),
        );
        await page.locator('.write-status.pending').waitFor({ state: 'detached' });
        ...
        const staleReorderResponse = page.waitForResponse(
          (response) =>
            response.url().endsWith('/api/bookmarks/commands') && response.status() === 409,
        );
        ...
        await page
          .getByRole('button', { name: 'Move Authoritative Bookmark', exact: true })
          .click();
        await page
          .getByLabel('Destination Folder')
          .selectOption('10000000-0000-4000-8000-000000000003');
        await page.locator('.move-dialog').getByRole('button', { name: 'Move' }).click();
        await page
          .getByRole('link', { name: /Authoritative Bookmark/ })
        ...
        if (
          ...
          throw new Error(
            `Folder Trash confirmation omitted subtree counts: ${folderConfirmation}`,
          );
        }
        ...
        await page.waitForFunction(
          () =>
            !document
              ...
              ?.textContent?.includes('Authoritative Bookmark'),
        );
      ...
      if (scenario === 'browsing') {
        ...
        await page
          ...
          .filter({ hasText: 'Duplicate Reference' })
          .waitFor();
        ...
        let failSlowSnapshot;
        let resolveSlowSnapshotIntercepted;
        const slowSnapshotIntercepted = new Promise((resolve) => {
          resolveSlowSnapshotIntercepted = resolve;
        });
        await page.route('**/api/bookmarks/snapshot', async (route) => {
          await new Promise((failRequest) => {
            failSlowSnapshot = failRequest;
            resolveSlowSnapshotIntercepted();
          });
          await route.abort('timedout');
        });
        await page.context().setOffline(false);
        await page.evaluate(() => window.dispatchEvent(new Event('online')));
        await slowSnapshotIntercepted;
        ...
        if (
          ...
          throw new Error(
            `Offline-without-snapshot status was contradictory: ${emptyOfflineStatus}`,
          );
        }
        await page.context().setOffline(false);
        await page.reload();
    ...
    console.log(`Local Worker ${scenario} verification passed.`);
  } finally {
    worker.kill('SIGTERM');
...
if (selectedScenario) {
  ...
  if (failures.length) {
    throw new Error(
      `Local Worker verification failed: ${failures
        .map(({ scenario, code, signal }) => `${scenario} (${signal ?? code})`)
        .join(', ')}`,
    );
  }
